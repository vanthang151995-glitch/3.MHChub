...
import { AlertTriangle, ArrowRight, BarChart2, CalendarDays, CheckCircle2, ClipboardCheck, Clock, FileClock, GraduationCap, Inbox, Layers3, Lock, RefreshCw, ShieldCheck, TrendingUp, UserCheck, XCircle, } from 'lucide-react';
...
function RejectModal({ onConfirm, onCancel }: {
    ...
    return (<div className="safety-reject-backdrop fixed inset-0 z-[1400] flex items-center justify-center bg-black/50 backdrop-blur-sm" role="presentation">
      ...
      <div className="safety-approval-workspace grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div aria-labelledby={`approval-kpi-tab-${filterStatus}`} className="safety-approval-list space-y-2" id="approval-kpi-list-panel" role="tabpanel">
            ...
            return (<button key={entry.id} onClick={() => selectEntry(entry.id)} aria-current={isSelected ? 'true' : undefined} className={`safety-approval-list-card w-full bg-card border rounded-xl p-4 shadow-sm cursor-pointer transition-all hover:shadow-md ${isSelected ? 'active border-[#1565c0] ring-1 ring-[#1565c0]/30' : 'border-border'}`} type="button">
